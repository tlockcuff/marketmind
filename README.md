...
- **Score-tiered sizing** — higher conviction = bigger position, volatile stocks = smaller
- **Market regime detection** — adapts thresholds and sizing for trending, choppy, or volatile conditions
- **Sector concentration limits** — max 3 positions per sector to avoid overexposure
...
- **Live terminal dashboard** — Rich TUI with positions, P/L, orders, logs, and API usage
...
- [Risk Management](docs/risk-management.md) — Position limits, stop-losses, PDT rules, and market regime filters
...
> Accounts under $25k are subject to the Pattern Day Trader rule (3 day trades per 5 rolling days). Marketmind tracks and enforces this automatically so you don't get flagged.
...
**Manages risk automatically** — Bracket orders set stop-loss and take-profit at entry. Trailing stops (ATR-based) ratchet up as price moves in your favor. Sector limits prevent overconcentration. A daily loss limit halts all trading if things go sideways.
...
./watch.sh              # Live dashboard (auto-starts bot)
...
| `SCAN_INTERVAL_MINUTES` | 5 | Minutes between Grok scans |
| `OPTIONS_ENABLED` | true | Toggle options trading |
| `OPTIONS_MAX_POSITION_PCT` | 2% | Max equity per options trade |

Sector concentration is capped at 3 positions per sector (configurable in `config/sectors.py`).
...
```
config/              Settings, holidays, sector map, logging
...
src/dashboard.py     Rich terminal UI
